	...
	$.fn.extend({
		passwordValidation: function(_options, _callback, _confirmcallback) {
			...
			var _defaults = {
				...
				maxRepeats: 10,		  //Maximum number of repeated alphanumeric characters in password dhgurAAAfjewd <- 3 A's
				maxConsecutive: 10,	  //Maximum number of alphanumeric characters from one set back to back
			...
			function buildPasswordRegex() {
				...
				if(_options.noUpper) 	cases.push({"regex": "(?=" + CHARSETS.upperCaseSet + ")",  																				"message": "Password can't contain an Upper Case Letter </br>"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.upperCaseSet + "][^" + CHARSETS.upperCaseSet + "]*").repeat(_options.minUpperCase) + ")", 	"message": "- Senha precisa conter pelo menos " + _options.minUpperCase + " letra maiúscula.</br>"});
				if(_options.noLower) 	cases.push({"regex": "(?=" + CHARSETS.lowerCaseSet + ")",  																				"message": "Password can't contain a Lower Case Letter"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.lowerCaseSet + "][^" + CHARSETS.lowerCaseSet + "]*").repeat(_options.minLowerCase) + ")", 	"message": "- Senha precisa conter pelo menos " + _options.minLowerCase + " letra minúscula.</br>"});
				if(_options.noDigit) 	cases.push({"regex": "(?=" + CHARSETS.digitSet + ")", 																					"message": "Password can't contain a Number"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.digitSet + "][^" + CHARSETS.digitSet + "]*").repeat(_options.minDigits) + ")", 			"message": "- Senha precisa conter pelo menos " + _options.minDigits + " dígito.</br>"});
				...
				cases.push({"regex":"[" + charsetToString() + "]{" + _options.minLength + ",}", "message":"- Senha precisa conter pelo menos  " + _options.minLength + " characteres.</br>"});
